...
class LocationImporter extends ItemImporter
{
    ...
    public function createLocationIfNotExists(array $row)
    {
        ...
        $this->item['location_code'] = trim($this->findCsvMatch($row, 'location_code'));
        ...
        if ($this->findCsvMatch($row, 'parent_id')) {
            $parent_id = trim($this->findCsvMatch($row, 'parent_id'));
            // Verify the parent ID exists
            if (Location::find($parent_id)) {
                $this->item['parent_id'] = $parent_id;
            } else {
                $this->log('Parent location ID ' . $parent_id . ' not found in database');
            }
        }
